...
class raster {
  ...
  createComponent(component) {
    ...
    let styles = component && component.instance && component.instance.style ? component.instance.style : null;

    if (component && component.instance && component.instance.tag) {
      ...
      if(component.instance) {
        // this can render multiplex
        // get tag this is actually probably a clutch right now for the
        // web raster, need to work the top branch and this into one
        // todo: make moar awesomeness
        let instanceType;
        let elementName = component.elementName;
        let oldInstance = component.instance;
        let newInstance;
        let tag;
        instance = _instances[component.instance.uuid];

        if(component.elementName.indexOf('Animated') > -1){
          try {
            instanceType = require(`../../components/animated`).Animated;
            elementName = elementName.replace('Animated','');
          } catch (e) {

          }
        } else {
          try {
            instanceType = require(`../../components/${component.elementName.toLowerCase()}`);
          } catch (e) {
        ...
        if(instanceType) {
          let newInstance = copyProps(oldInstance, new instanceType[elementName]());
          tag = newInstance.tag(instance);
        }
  ...
  syncState(component, viewParent) {
    // console.log('sync state', _instances[component.instance.uuid]);
...
function copyProps(flatInstance, instance) {
  for (var k in flatInstance) {
      if (flatInstance.hasOwnProperty(k)) {
        instance[k] = flatInstance[k];
      }
  }
  return instance;
}
